...
export default store => next => action => {
  ...
  if(!action.createdInstanceLogs) {
    const instanceCreatedLog = state.contracts.ethernaut.events.LevelInstanceCreatedLog(query.filter, query.range)
  ...
  if(!action.completedLevelLogs) {
    const levelCompletedLog = state.contracts.ethernaut.events.LevelCompletedLog(query.filter, query.range)
